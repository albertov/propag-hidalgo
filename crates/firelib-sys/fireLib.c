...
int Fire_FuelCombustion(catalog, model)
...
size_t model;           /* fuel model id number         [0-maxModels] */
{
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelCombustion(): fuel model %d doesn't exist in fuel "
            "catalog \"%s\".",
  ...
  for (particle = 0; particle < Fuel_Particles(catalog, model); particle++) {
    ...
    if (lifeArea[life] > Smidgen) {
      ...
      sizeClassAreaWtg[life][size] += Fuel_AreaWtg(catalog, model, particle);
    }
  }
  ...
  for (particle = 0; particle < Fuel_Particles(catalog, model); particle++) {
    if (Fuel_Live(catalog, model, particle))
      fineLive += Fuel_Load(catalog, model, particle) *
...
int Fire_SpreadNoWindNoSlope(catalog, model, moisture)
FuelCatalogPtr catalog;         /* FuelCatalogData instance pointer           */
...
double moisture[FIRE_MCLASSES]; /* array of fuel moistures   (fractions) */
{
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_SpreadNoWindNoSlope(): fuel model %d doesn't exist in fuel "
            "catalog \"%s\".",
            model, FuelCat_Name(catalog));
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  for (particle = 0; particle < Fuel_Particles(catalog, model); particle++) {
    ...
    if (Fuel_Live(catalog, model, particle) == FIRE_LIFE_DEAD) {
      ...
      wfmd += moisture[mclass] * Fuel_SigmaFactor(catalog, model, particle) *
              Fuel_Load(catalog, model, particle);
    } else {
      nlive++;
  ...
  if (nlive) {
    fdmois = (Fuel_FineDead(catalog, model) > Smidgen)
                 ? (wfmd / Fuel_FineDead(catalog, model))
                 : 0.;

    lifeMext[FIRE_LIFE_LIVE] = ((Fuel_LiveMextFactor(catalog, model) *
                                 (1.0 - fdmois / Fuel_Mext(catalog, model))) -
                                0.226);
  ...
  for (particle = 0; particle < Fuel_Particles(catalog, model); particle++) {
    ...
    rbQig += qig * Fuel_AreaWtg(catalog, model, particle) *
  ...
  for (life = 0; life < FIRE_LIFE_CATS; life++) {
    ...
    if (lifeMext[life] > Smidgen) {
      ...
      lifeEtaM[life] = 1.0 - 2.59 * ratio + 5.11 * ratio * ratio -
    ...
    Fuel_RxIntensity(catalog, model) +=
        Fuel_LifeRxFactor(catalog, model, life) * lifeEtaM[life];
  }

  /* Calculate heat per unit area from rx intensity and residence time. */
  Fuel_HeatPerUnitArea(catalog, model) =
      Fuel_RxIntensity(catalog, model) * Fuel_ResidenceTime(catalog, model);

  /* Calculate no-wind, no-slope spread rate. */
  Fuel_Spread0(catalog, model) = (rbQig > Smidgen)
                                     ? Fuel_RxIntensity(catalog, model) *
                                           Fuel_PropFlux(catalog, model) / rbQig
                                     : 0.;
...
int Fire_SpreadWindSlopeMax(catalog, model, windFpm, windDeg, slope, aspect)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model number            [0-maxModels] */
...
double aspect;          /* aspect (downslope)   azimuth (compass deg) */
{
  ...
  if (!Equal(Fuel_Slope(catalog, model), slope)) {
    Fuel_PhiSlope(catalog, model) = Fuel_SlopeK(catalog, model) * slope * slope;
  ...
  if (!Equal(Fuel_WindSpeed(catalog, model), windFpm)) {
    Fuel_PhiWind(catalog, model) =
        (windFpm < Smidgen) ? 0.
                            : Fuel_WindK(catalog, model) *
                                  pow(windFpm, Fuel_WindB(catalog, model));
  ...
  /* Situation 6: wind blows cross slope. */
    ...
    splitDeg =
        (upslope <= windDeg) ? windDeg - upslope : 360. - upslope + windDeg;
  ...
  if (checkWindLimit) {
    ...
    if (effectiveWind > maxWind) {
      phiEw = (maxWind < Smidgen)
                  ? 0.
                  : Fuel_WindK(catalog, model) *
                        pow(maxWind, Fuel_WindB(catalog, model));
  ...
  Fuel_AzimuthMax(catalog, model) = Fuel_AzimuthAny(catalog, model) =
      azimuthMax;
  Fuel_LwRatio(catalog, model) = lwRatio;
  Fuel_Eccentricity(catalog, model) = eccentricity;
  Fuel_BackingRate(catalog, model) = backing;
...
int Fire_SpreadAtAzimuth(catalog, model, azimuth, which)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model number            [0-maxModels] */
...
size_t which;           /* FIRE_NONE | FIRE_BYRAMS | FIRE_FLAME | FIRE_SCORCH */
{
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_SpreadAtAzimuth(): fuel model %d doesn't exist in fuel "
            "catalog \"%s\".",
            model, FuelCat_Name(catalog));
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  /* Situation 3: wind and/or slope and azimuth not in max spread direction */
    ...
    if (denom > Smidgen) {
      ...
      Fuel_SpreadAny(catalog, model) =
          Fuel_Spread0(catalog, model); /* BUG FIX */
    }
  }
  Fuel_AzimuthAny(catalog, model) = azimuth;
  ...
  if (which) {
    ...
    byrams = Fuel_ResidenceTime(catalog, model) *
             Fuel_SpreadAny(catalog, model) * Fuel_RxIntensity(catalog, model) /
             60.;
...
int Fire_FlameScorch(catalog, model, which)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model number            [0-maxModels] */
size_t which;           /* FIRE_NONE | FIRE_BYRAMS | FIRE_FLAME | FIRE_SCORCH */
{
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FlameScorch(): fuel model %d doesn't exist in fuel catalog "
            "\"%s\".",
            model, FuelCat_Name(catalog));
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }

  byrams = Fuel_ResidenceTime(catalog, model) * Fuel_SpreadAny(catalog, model) *
           Fuel_RxIntensity(catalog, model) / 60.;
...
int Fire_FlameLengthTable(catalog, flameClasses, flameStep)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
...
double flameStep;       /* flame length step value per class          */
{
  ...
  if ((FuelCat_FlameArray(catalog) =
           (double *)calloc(flameClasses, sizeof(double))) == NULL) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FlameLengthTable(): unable to allocate flame length table "
            "with %d classes of %f feet.",
...
FuelCatalogPtr Fire_FuelCatalogCreate(name, maxModels) const
    char *const name; /* FuelCatalogData instance name */
size_t maxModels;     /* maximum modelId allowed in this catalog */
{
  ...
  if ((catalog = (FuelCatalogPtr)malloc(sizeof(FuelCatalogData))) == NULL) {
    fprintf(stderr,
            "Fire_FuelCatalogCreate(): unable to allocate fuel catalog \"%s\" "
            "object.\n",
            name);
    return (NULL);
  }
  ...
  if ((FuelCat_Name(catalog) = strdup(name)) == NULL) {
    fprintf(stderr,
            "Fire_FuelCatalogCreate(): unable to duplicate fuel catalog \"%s\" "
            "name.\n",
            name);
    free(catalog);
    return (NULL);
  ...
  if ((FuelCat_ModelArray(catalog) = (FuelModelPtr *)calloc(
           FuelCat_MaxModels(catalog) + 1, sizeof(FuelModelPtr))) == NULL) {
    fprintf(stderr,
            "Fire_FuelCatalogCreate(): unable to allocate fuel catalog \"%s\" "
            "with %d fuel models.\n",
            name, maxModels);
...
FuelCatalogPtr Fire_FuelCatalogCreateStandard(name, maxModels) const
    char *const name; /* FuelCatalogData instance name */
size_t maxModels;     /* maximum modelId allowed in this catalog */
{
  FuelCatalogPtr catalog;
  ...
  static StandardParticle P[39] = {
      {1, FIRE_TYPE_DEAD, 0.0340, 3500.},  {2, FIRE_TYPE_DEAD, 0.0920, 3000.},
      {2, FIRE_TYPE_DEAD, 0.0460, 109.},   {2, FIRE_TYPE_DEAD, 0.0230, 30.},
      {2, FIRE_TYPE_HERB, 0.0230, 1500.},  {3, FIRE_TYPE_DEAD, 0.1380, 1500.},
      {4, FIRE_TYPE_DEAD, 0.2300, 2000.},  {4, FIRE_TYPE_DEAD, 0.1840, 109.},
      {4, FIRE_TYPE_DEAD, 0.0920, 30.},    {4, FIRE_TYPE_WOOD, 0.2300, 1500.},
      {5, FIRE_TYPE_DEAD, 0.0460, 2000.},  {5, FIRE_TYPE_DEAD, 0.0230, 109.},
      {5, FIRE_TYPE_WOOD, 0.0920, 1500.},  {6, FIRE_TYPE_DEAD, 0.0690, 1750.},
      {6, FIRE_TYPE_DEAD, 0.1150, 109.},   {6, FIRE_TYPE_DEAD, 0.0920, 30.},
      {7, FIRE_TYPE_DEAD, 0.0520, 1750.},  {7, FIRE_TYPE_DEAD, 0.0860, 109.},
      {7, FIRE_TYPE_DEAD, 0.0690, 30.},    {7, FIRE_TYPE_WOOD, 0.0170, 1550.},
      {8, FIRE_TYPE_DEAD, 0.0690, 2000.},  {8, FIRE_TYPE_DEAD, 0.0460, 109.},
      {8, FIRE_TYPE_DEAD, 0.1150, 30.},    {9, FIRE_TYPE_DEAD, 0.1340, 2500.},
      {9, FIRE_TYPE_DEAD, 0.0190, 109.},   {9, FIRE_TYPE_DEAD, 0.0070, 30.},
      {10, FIRE_TYPE_DEAD, 0.1380, 2000.}, {10, FIRE_TYPE_DEAD, 0.0920, 109.},
      {10, FIRE_TYPE_DEAD, 0.2300, 30.},   {10, FIRE_TYPE_WOOD, 0.0920, 1500.},
      {11, FIRE_TYPE_DEAD, 0.0690, 1500.}, {11, FIRE_TYPE_DEAD, 0.2070, 109.},
      {11, FIRE_TYPE_DEAD, 0.2530, 30.},   {12, FIRE_TYPE_DEAD, 0.1840, 1500.},
      {12, FIRE_TYPE_DEAD, 0.6440, 109.},  {12, FIRE_TYPE_DEAD, 0.7590, 30.},
      {13, FIRE_TYPE_DEAD, 0.3220, 1500.}, {13, FIRE_TYPE_DEAD, 1.0580, 109.},
  ...
  for (m = 0; m < 14; m++) {
    if (Fire_FuelModelCreate(catalog, m, M[m].name, M[m].desc, M[m].depth,
                             M[m].mext, adjust,
                             M[m].maxParticles) != FIRE_STATUS_OK) {
      fprintf(stderr, "%s\n", FuelCat_Error(catalog));
      Fire_FuelCatalogDestroy(catalog);
      return (NULL);
    }
...
int Fire_FuelCatalogDestroy(catalog)
...
int Fire_FuelModelCreate(catalog, model, name, desc, depth, mext, adjust,
                         maxParticles)
...
size_t maxParticles;    /* maximum number of fuel model particles     */
{
  ...
  if (model > FuelCat_MaxModels(catalog)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %d exceeds fuel "
            "catalog \"%s\" range [0..%d].",
  ...
  if (depth < Smidgen) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %d depth %5.4f "
            "is too small.",
            name, model, depth);
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  if (mext < Smidgen) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %d extinction "
            "moisture %5.4f is too small.",
            name, model, mext);
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  if ((FuelCat_ModelPtr(catalog, model) =
    ...
    Fire_FuelModelDestroy(catalog, model);
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): unable to allocate fuel model \"%s\" "
            "number %d for fuel catalog \"%s\".",
...
int Fire_FuelModelDestroy(catalog, model)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model id number         [0-maxModels] */
{
  size_t particle;
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelDestroy(): fuel model %d doesn't exist in fuel "
            "catalog \"%s\".",
            model, FuelCat_Name(catalog));
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  if (Fuel_ParticleArray(catalog, model)) {
    for (particle = 0; particle < Fuel_MaxParticles(catalog, model);
         particle++) {
...
int Fire_FuelModelExists(catalog, model)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model id number         [0-maxModels] */
{
  /* Validate the model number. */
  if (model > FuelCat_MaxModels(catalog) || !FuelCat_ModelPtr(catalog, model))
    return (int)0;
...
int Fire_FuelParticleAdd(catalog, model, type, load, savr, dens, heat, stot,
                         seff)
FuelCatalogPtr catalog; /* FuelCatalogData instance pointer           */
size_t model;           /* fuel model id number         [0-maxModels] */
...
{
  static double Size_boundary[FIRE_SIZE_CLASSES] = {1200., 192., 96.,
                                                    48.,   16.,  0.};
  ...
  if (!Fire_FuelModelExists(catalog, model)) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): fuel model %d doesn't exist in fuel "
            "catalog \"%s\".",
            model, FuelCat_Name(catalog));
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  if (type != FIRE_TYPE_DEAD && type != FIRE_TYPE_HERB &&
      type != FIRE_TYPE_WOOD) {
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): fuel model %d type value (arg #3) is not "
            "FIRE_TYPE_DEAD, FIRE_TYPE_HERB, or FIRE_TYPE_WOOD.",
            model);
    return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
  }
  ...
  if ((Fuel_ParticlePtr(catalog, model, particle) =
    ...
    sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): unable to allocate fuel particle to fuel "
            "model \"%s\" number %d in fuel catalog \"%s\".",
