...
#define Fuel_Model(catalog, model) ((catalog)->modelPtr[(model)]->modelId)

#define Fuel_Name(catalog, model) ((catalog)->modelPtr[(model)]->name)

#define Fuel_Desc(catalog, model) ((catalog)->modelPtr[(model)]->desc)

#define Fuel_Depth(catalog, model) ((catalog)->modelPtr[(model)]->depth)

#define Fuel_Mext(catalog, model) ((catalog)->modelPtr[(model)]->mext)
...
#define Fuel_Particles(catalog, model) ((catalog)->modelPtr[(model)]->particles)
...
#define Fuel_FineDead(catalog, model) ((catalog)->modelPtr[(model)]->fineDead)

#define Fuel_LiveMextFactor(catalog, model)                                    \
  ((catalog)->modelPtr[(model)]->liveFactor)

#define Fuel_BulkDensity(catalog, model) ((catalog)->modelPtr[(model)]->rhob)

#define Fuel_ResidenceTime(catalog, model) ((catalog)->modelPtr[(model)]->taur)

#define Fuel_PropFlux(catalog, model) ((catalog)->modelPtr[(model)]->propFlux)

#define Fuel_SlopeK(catalog, model) ((catalog)->modelPtr[(model)]->slopeK)

#define Fuel_WindB(catalog, model) ((catalog)->modelPtr[(model)]->windB)

#define Fuel_WindE(catalog, model) ((catalog)->modelPtr[(model)]->windE)

#define Fuel_WindK(catalog, model) ((catalog)->modelPtr[(model)]->windK)

/* Fuel model fire behavior variable macros. */
#define Fuel_RxIntensity(catalog, model) ((catalog)->modelPtr[(model)]->rxInt)

#define Fuel_Spread0(catalog, model) ((catalog)->modelPtr[(model)]->spread0)

#define Fuel_HeatPerUnitArea(catalog, model)                                   \
  ((catalog)->modelPtr[(model)]->hpua)

#define Fuel_SpreadMax(catalog, model) ((catalog)->modelPtr[(model)]->spreadMax)

#define Fuel_AzimuthMax(catalog, model)                                        \
  ((catalog)->modelPtr[(model)]->azimuthMax)

#define Fuel_SpreadAny(catalog, model) ((catalog)->modelPtr[(model)]->spreadAny)
...
#define Fuel_LwRatio(catalog, model) ((catalog)->modelPtr[(model)]->lwRatio)

#define Fuel_Eccentricity(catalog, model)                                      \
  ((catalog)->modelPtr[(model)]->eccentricity)

#define Fuel_BackingRate(catalog, model) ((catalog)->modelPtr[(model)]->backing)

#define Fuel_PhiWind(catalog, model) ((catalog)->modelPtr[(model)]->phiW)

#define Fuel_PhiSlope(catalog, model) ((catalog)->modelPtr[(model)]->phiS)

#define Fuel_PhiEffWind(catalog, model) ((catalog)->modelPtr[(model)]->phiEw)

#define Fuel_WindLimit(catalog, model) ((catalog)->modelPtr[(model)]->wLimit)

#define Fuel_ByramsIntensity(catalog, model)                                   \
  ((catalog)->modelPtr[(model)]->byrams)

#define Fuel_FlameLength(catalog, model) ((catalog)->modelPtr[(model)]->flame)

#define Fuel_ScorchHeight(catalog, model) ((catalog)->modelPtr[(model)]->scorch)

/* Fuel model environment variable macros. */
#define Fuel_EnvMoisture(catalog, model, mclass)                               \
  ((catalog)->modelPtr[(model)]->moisture[(mclass)])

#define Fuel_WindSpeed(catalog, model) ((catalog)->modelPtr[(model)]->windFpm)

#define Fuel_WindDir(catalog, model) ((catalog)->modelPtr[(model)]->windDeg)

#define Fuel_Slope(catalog, model) ((catalog)->modelPtr[(model)]->slope)

#define Fuel_Aspect(catalog, model) ((catalog)->modelPtr[(model)]->aspect)
