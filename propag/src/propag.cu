...
__device__ inline bool offset_shared_ix(
...
__device__ inline bool offset_ix(
    const GeoReference &geo_ref,
    int2 offset,
    size_t *result
  ) {
...
__device__ inline Point preload_point(
    const GeoReference &geo_ref,
    int2 offset,
    const float *speed_max,
    const float *azimuth_max,
...
__device__ inline bool similar_fires(const FireSimpleCuda &a, const FireSimpleCuda &b) {
  return (
      abs(a.speed_max - b.speed_max) < 1.0 &&
      abs(a.azimuth_max - b.azimuth_max) < (5.0 / 2*PI) &&
      abs(a.eccentricity - b.eccentricity) < 0.1
  );
}

__device__ inline int signum(int val) {
      return (0 < val) - (val < 0);
}
...
__device__ inline float time_to(
    const GeoReference &geo_ref,
    const PointRef &from,
    const uint2 to
) {
    ...
    for (int i=-1; i<2; i++) {
      ...
      for (int j=-1; j<2; j++) {
        ...
        if (reference.pos_x = pos.x && reference.pos_y == pos.y) {
          ...
          uint2 possible_blockage_pos =
            neighbor_in_direction(
                idx_2d,
                make_uint2(reference.pos_x, reference.pos_y)
                );
          Point possible_blockage = shared[compute_shared_ix(possible_blockage_pos)];
        ...
        if (!is_fire_null(fire) && reference.time != MAX_TIME) {
          ...
          if (!similar_fires(fire, reference.fire)) {
            ...
            if (r.pos_x != idx_2d.x && r.pos_y != idx_2d.y && similar_fires(fire, r.fire)) {
              reference = r;
            };
          };
